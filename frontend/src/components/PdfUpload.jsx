...
const PdfUpload = ({ onUpload, isProcessing }) => {
  ...
  const handleUrlSubmit = async (e) => {
    ...
    try {
      const result = await uploadPdfLink(url); // real API call
      onUpload(result);
      setUrl('');
    } catch (err) {
      setError(err.message || 'Error uploading PDF from URL');
    }
  };
  ...
  return (
    <div className="space-y-3">
      <Tabs defaultValue="file" className="w-full">
          ...
          <form onSubmit={handleFileSubmit} className="space-y-3">
            ...
            {progress > 0 && progress < 100 && (
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-gray-500 text-right">Uploading... {progress}%</p>
              </div>
            )}

            <Button type="submit" className="w-full h-8 text-xs" disabled={!file || isProcessing} size="sm">
              {isProcessing ? (
                <>
        ...
        <TabsContent value="url" className="mt-3">
          <form onSubmit={handleUrlSubmit} className="space-y-3">
            ...
            />
            <p className="text-[10px] text-gray-500">Enter a direct URL to a PDF file</p>

            <Button type="submit" className="w-full h-8 text-xs" disabled={!url || isProcessing} size="sm">
              {isProcessing ? (
                <>
                  <Loader2 className="mr-1.5 h-3 w-3 animate-spin" />
                  Processing...
      ...
      {error && (
        <div className="mt-2 p-2 text-xs text-red-600 bg-red-50 rounded">{error}</div>
      )}
    </div>
  );
};
