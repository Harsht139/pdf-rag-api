...
async def create_chat_session(
    ...
    doc = supabase.table("documents").select("id").eq("id", session_data.document_id).execute()
    ...
    session = {
        ...
        "title": session_data.title or f"Chat about {doc.data[0].get('title', 'Document')}",
...
async def create_chat_message(
    ...
    session = sb.table("chat_sessions").select("*").eq("id", session_id).eq("user_id", current_user.get('id')).execute()
    ...
    try:
        ...
        sb.table("chat_sessions").update({"updated_at": datetime.utcnow().isoformat()}).eq("id", session_id).execute()
...
async def get_chat_messages(
    ...
    # Verify session exists and user has access
    session = sb.table("chat_sessions").select("id").eq("id", session_id).eq("user_id", current_user.get('id')).execute()
    if not session.data or len(session.data) == 0:
        raise HTTPException(status_code=404, detail="Chat session not found")
