...
async def download_file(url: str, headers: Optional[Dict[str, str]] = None) -> Tuple[bytes, str, Dict[str, Any]]:
    """
...
async def process_google_drive_url(url: str, metadata: Dict[str, Any]) -> Tuple[str, str, Dict[str, Any]]:
...
async def process_onedrive_url(url: str, metadata: Dict[str, Any]) -> Tuple[str, str, Dict[str, Any]]:
...
async def process_dropbox_url(url: str, metadata: Dict[str, Any]) -> Tuple[str, str, Dict[str, Any]]:
...
def get_filename_from_url(url: str, response: aiohttp.ClientResponse) -> str:
    ...
    if not any(filename.lower().endswith(ext) for ext in ['.pdf', '.doc', '.docx', '.txt']):
        content_type = response.content_type
